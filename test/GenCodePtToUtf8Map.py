...
with open('./src/Utf8Map.hpp', 'w') as outFile:
	outFile.write('#pragma once\n')
	outFile.write('\n')

	outFile.write('const char32_t gk_Utf8MapBegins[] =\n')
	outFile.write('{\n')
	for item in TEST_BEGINS:
		outFile.write('\t{},\n'.format(item))
	outFile.write('};\n\n')

	outFile.write('const unsigned char gk_Utf8Map[][{}][4] =\n'.format(TEST_RANGE))
	outFile.write('{\n')
	for item in TEST_BEGINS:
		outFile.write('\t{\n')
		for i in range(item, item + TEST_RANGE):
			...
			s = '{}'
			try:
				c = v.decode('utf-32be').encode('utf-8')
				s = GetArrayStr(c)
			except:
				pass
			outFile.write('\t\t')
			...
			if i == 0x80:
				outFile.write(' #2B MIN')
			if i == 0x800:
				outFile.write(' #3B MIN')
			if i == 0x10000:
				outFile.write(' #4B MIN')
			outFile.write('\n')
		outFile.write('\t},\n')
	outFile.write('};\n\n')
